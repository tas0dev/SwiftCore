    ...
    loop {
        ...
        if sender != 0 && (len as usize) >= (size_of::<DiskRequest>() - 512) {
            ...
            let has_storage_cap = swiftlib::capability::check_thread_capability(sender, "device.storage")
                ...
                .unwrap_or(false);
            if !has_storage_cap {
                println!("[DISK] Rejecting request from thread {} (missing device.storage)", sender);
            ...
            // 追加の防御（互換目的）:
            // 既存実装が privilege で Service/Core を想定しているため、ここでも不正な呼び出しを弾く。
            let sender_privilege = task::get_thread_privilege(sender);
            if sender_privilege > 1 {
                println!(
                    "[DISK] Rejecting request from unprivileged thread {} (privilege={})",
                    sender, sender_privilege
                );
                let resp = DiskResponse {
                    status: -13, // EACCES
                    len: 0,
