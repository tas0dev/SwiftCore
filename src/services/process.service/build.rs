...
fn main() {
    // host PoC では mochiOS 固有のリンク設定を無効化する
    if std::env::var("MOCHI_HOST_POC").is_ok() {
        println!("cargo:warning=MOCHI_HOST_POC set; skipping mochiOS linker flags in services/process/build.rs");
        return;
    }
    ...
    // std の unwind が libgcc_s を要求するため、libg.a を libgcc_s.a として提供する
