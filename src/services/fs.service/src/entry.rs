...
fn check_caller_capability(caller_tid: u64, capability: &str) -> Result<(), i64> {
    match swiftlib::capability::check_thread_capability(caller_tid, capability) {
        Ok(true) => Ok(()),
                    ...
                    if fd < MAX_HANDLES && unsafe { HANDLES[fd].used } {
                        ...
                        unsafe {
                            if let Some(ref mut fs) = MOUNTED_FS {
                                ...
                                let actual_len = core::cmp::min(write_len, 128);
                                let data = &req.path[..actual_len]; // FsRequest に書き込みバッファが無いため暫定で path を利用

                                match fs.write(inode, offset, data) {
                                    Ok(bytes_written) => {
                                        resp.status = bytes_written as i64;
