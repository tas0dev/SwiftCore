...
/// mochiOS VGA/フレームバッファ（`swiftlib::vga`）バックエンド
///
/// - mochiOS: syscall 経由で VRAM をマップし、`flush()` で転送
/// - Linux host: `backend-mochios-vga-hosted` を有効にすると `swiftlib/hosted-vga` で模擬
...
impl FramebufferBackend for MochiVgaBackend {
    async fn init(&mut self) -> Result<FramebufferInfo, BackendError> {
        ...
        let fb_info = {
            let mut fb_info = swiftlib::vga::get_info();
            if fb_info.is_none() {
                ...
                let height = std::env::var("VGA_HEIGHT")
                    ...
                    .parse::<u32>()
                    .unwrap_or(480);
                let _ = swiftlib::vga::host_init_framebuffer(width, height);
                fb_info = swiftlib::vga::get_info();
            }
            fb_info
        };

        #[cfg(not(feature = "backend-mochios-vga-hosted"))]
        let fb_info = swiftlib::vga::get_info();

        let fb_info = fb_info.ok_or_else(|| {
            BackendError::InitializationFailed("swiftlib::vga::get_info failed".to_string())
        })?;

        let fb_ptr = swiftlib::vga::map_framebuffer().ok_or_else(|| {
            BackendError::MemoryMapFailed("swiftlib::vga::map_framebuffer failed".to_string())
        })?;
